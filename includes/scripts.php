...
function tsatu_scripts() {
  ...
  if (WP_ENV === 'development') {
    $assets = array(
      //'css'       => '/assets/css/main' . get_current_blog_id() . '.css',
    ...
    $assets     = array(
      //'css'       => '/assets/css/main' . get_current_blog_id() . '.min.css?' . $assets['assets/css/main.min.css']['hash'],
  ...
  wp_enqueue_style('tsatu_style', get_template_directory_uri() . $assets['css'], false, null);
...
function tsatu_jquery_local_fallback($src, $handle = null) {
...
add_action('wp_head', 'tsatu_jquery_local_fallback');
 ...
 */
function tsatu_google_analytics() { ?>
<script>
...
if (GOOGLE_ANALYTICS_ID && (WP_ENV !== 'production' || !current_user_can('manage_options'))) {
  add_action('wp_footer', 'tsatu_google_analytics', 20);
}
